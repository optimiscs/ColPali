...
class ColModelTraining:
    ...
    def save(self, max_shard_size: str = "1GB"):
        """
        Save the model with its training config, as well as the tokenizer and processor if provided.
        Uses sharded save to avoid safetensors header size limit with large models.
        """
        self.model.save_pretrained(self.config.output_dir, max_shard_size=max_shard_size)
