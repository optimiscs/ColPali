...
# USE_STREAMING = os.environ.get("USE_STREAMING", "0") == "1"
USE_LOCAL_DATASET = 0
USE_STREAMING = 0

def load_train_set() -> ColPaliEngineDataset:
    base_path = "./data_dir/" if USE_LOCAL_DATASET else "vidore/"
    dataset = load_dataset(base_path + "colpali_train_set", split="train", streaming=USE_STREAMING)
...
def load_eval_set(dataset_path) -> ColPaliEngineDataset:
    dataset = load_dataset(dataset_path, split="test", streaming=USE_STREAMING)
...
def load_train_set_ir(num_negs=0) -> ColPaliEngineDataset:
    ...
    corpus_data = load_dataset(base_path + "colpali-corpus", split="train", streaming=USE_STREAMING)
    corpus = Corpus(corpus_data=corpus_data, doc_column_name="image")

    dataset = load_dataset(base_path + "colpali-queries", split="train", streaming=USE_STREAMING)
...
def load_train_set_detailed() -> DatasetDict:
    ...
    for path in ds_paths:
        ...
        ds = cast(Dataset, load_dataset(cpath, split="train", streaming=USE_STREAMING))
        if "arxivqa" in path:
            # subsample 10k
...
def load_docmatix_ir_negs() -> Tuple[DatasetDict, Dataset, str]:
    ...
    dataset = cast(Dataset, load_dataset(base_path + "docmatix-ir", split="train", streaming=USE_STREAMING))
    ...
    anchor_ds = cast(Dataset, load_dataset(base_path + "Docmatix", "images", split="train", streaming=USE_STREAMING))
...
def load_wikiss() -> Tuple[DatasetDict, Dataset, str]:
    """Returns the query dataset, then the anchor dataset with the documents, then the dataset type"""
    base_path = "./data_dir/" if USE_LOCAL_DATASET else "Tevatron/"
    dataset = cast(Dataset, load_dataset(base_path + "wiki-ss-nq", data_files="train.jsonl", split="train", streaming=USE_STREAMING))
    ...
    base_path = "./data_dir/" if USE_LOCAL_DATASET else "HuggingFaceM4/"
    anchor_ds = cast(Dataset, load_dataset(base_path + "wiki-ss-corpus", split="train", streaming=USE_STREAMING))
...
def load_docvqa_dataset() -> DatasetDict:
    if USE_LOCAL_DATASET:
        dataset_doc = cast(Dataset, load_dataset("./data_dir/DocVQA", "DocVQA", split="validation", streaming=USE_STREAMING))
        dataset_doc_eval = cast(Dataset, load_dataset("./data_dir/DocVQA", "DocVQA", split="test", streaming=USE_STREAMING))
        dataset_info = cast(Dataset, load_dataset("./data_dir/DocVQA", "InfographicVQA", split="validation", streaming=USE_STREAMING))
        dataset_info_eval = cast(Dataset, load_dataset("./data_dir/DocVQA", "InfographicVQA", split="test", streaming=USE_STREAMING))
    else:
        dataset_doc = cast(Dataset, load_dataset("lmms-lab/DocVQA", "DocVQA", split="validation", streaming=USE_STREAMING))
        dataset_doc_eval = cast(Dataset, load_dataset("lmms-lab/DocVQA", "DocVQA", split="test", streaming=USE_STREAMING))
        dataset_info = cast(Dataset, load_dataset("lmms-lab/DocVQA", "InfographicVQA", split="validation", streaming=USE_STREAMING))
        dataset_info_eval = cast(Dataset, load_dataset("lmms-lab/DocVQA", "InfographicVQA", split="test", streaming=USE_STREAMING))
...
def load_multi_qa_datasets() -> List[DatasetDict]:
    ...
    for args in dataset_args:
        ...
        dataset = load_dataset(*args, streaming=USE_STREAMING)
        if "test" in dataset:
...
class TestSetFactory:
    ...
    def __call__(self, *args, **kwargs):
        dataset = load_dataset(self.dataset_path, split="test", streaming=USE_STREAMING)
        return dataset
