from .paligemma import BiPali, BiPaliProcessor, BiPaliProj, ColPali, ColPaliProcessor
from .qwen2 import BiQwen2, BiQwen2Processor, ColQwen2, ColQwen2Processor
from .qwen2_5 import BiQwen2_5, BiQwen2_5_Processor, ColQwen2_5, ColQwen2_5_Processor

try:
    from .gemma3 import BiGemma3, BiGemmaProcessor3, ColGemma3, ColGemmaProcessor3
except ModuleNotFoundError:
    pass

try:
    from .idefics3 import (
        BiIdefics3,
        BiIdefics3Processor,
        ColIdefics3,
        ColIdefics3Processor,
    )
except ModuleNotFoundError:
    pass

try:
    from .modernvbert import (
        BiModernVBert,
        BiModernVBertProcessor,
        ColModernVBert,
        ColModernVBertProcessor,
    )
except ModuleNotFoundError:
    # Some local environments pin a transformers build without
    # `transformers.conversion_mapping`, which only affects ModernVBert.
    # Keep ColPali/ColQwen imports usable instead of failing package import.
    pass

try:
    from .qwen3 import (
        BiQwen3,
        BiQwen3Processor,
        ColQwen3,
        ColQwen3Processor,
        ColQwen3VLEmbedding,
        ColQwen3VLEmbeddingProcessor,
    )
except ModuleNotFoundError:
    pass

try:
    from .qwen3_5 import (
        BiQwen3_5,
        BiQwen3_5Processor,
        ColQwen3_5,
        ColQwen3_5Processor,
    )
except ModuleNotFoundError:
    pass

try:
    from .qwen_omni import ColQwen2_5Omni, ColQwen2_5OmniProcessor
except ModuleNotFoundError:
    pass
